...
export class ApiAuthService {
    ...
    /**
     * Ham tao cay menu khi co cau truc id, parent
     */
    createTreeMenu = (arrIn: any, idKey: string, parentKey: string, startWith?: any, level?: number) => {
        ...
        if (roots && roots.length > 0) {
            roots.forEach(el => {
                el.$level = myLevel;
                el.subs = this.createTreeMenu(arrIn, idKey, parentKey, el[idKey], myLevel + 1)
            })
            return roots;
        } else {
     ...
     */
    deleteToken() {
        this.apiStorage.deleteToken(); //xóa thông tin lưu trữ token
        this.tokenObject = null; //xóa thông tin token
        this.userInfo = null; //xóa thông tin user
    }
