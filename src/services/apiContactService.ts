...
export class ApiContactService {
            ...
            if (users && this.apiStorage.getToken()) {
                ...
                users.forEach(async el => {
                    if (!el.avatar || isRenew) { //chua co avatar (da tung luu hoac luu moi)
                        if (!el.image) {
                            el.image = ApiStorageService.mediaServer + "/get-private?func=avatar&user=" + el.username + "&token=" + this.apiStorage.getToken();
    ...
    listContactsFromSmartPhone() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //console.log('Danh ba la', this.contacts);
           
            try {
                let data = await this.contacts
