...
export class DynamicFormMobilePage {
  ...
  onClickHeader(btn){
    //dùng cho hàm callback hoặc CLOSE 
    //để truyền dữ liệu ngược lại cho dismiss và callback
    btn.next_data = {
      step: this.step,
      button: btn
    }
    ...
    if (valid) {
      ...
      if (btn.url) {
        ...
        if (keyResults) {
          ...
          loading.present();
          //console.log('kq',keyResults);
          this.apiAuth.postDynamicForm(btn.url, keyResults, btn.token)
            .then(data => {
              btn.next_data = {
                step: this.step,
