...
export class OwnerImagesPage {
  ...
  dynamicMediasOrigin: any = {
    ...
    ,actions:{
        ...
        , buttons: [
             {name:"Lưu", icon: "share-alt", color:"primary", url:ApiStorageService.mediaServer + "/upload-files", method: "FORM-DATA", token:true , next:"SAVE"}
        ]
    }
};
  ...
  getMyImages(){
    ...
    this.apiAuth.getDynamicUrl(ApiStorageService.mediaServer + "/list-files?limit=12&offset=0", true)
    .then(data=>{
      this.myImages = data;
      this.myImages.forEach(el=>{
        el.image= encodeURI(ApiStorageService.mediaServer + "/get-file/" + el.url);
      })
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            //gui 
            if (this.func==='avatar'){
              let json_data = {image: ApiStorageService.mediaServer + "/get-file/" + encodeURI(item.url)}
              this.apiAuth.postDynamicForm(ApiStorageService.authServer+"/ext-auth/save-user-info",json_data,true)
              .then(data=>{})
              .catch(err=>{})
            }else{
              if (this.func==='background'){
                let json_data = {background: ApiStorageService.mediaServer + "/get-file/" + encodeURI(item.url)}
                this.apiAuth.postDynamicForm(ApiStorageService.authServer+"/ext-auth/save-user-info",json_data,true)
                .then(data=>{})
                .catch(err=>{})
              }
            ...
            this.apiAuth.postDynamicForm(ApiStorageService.mediaServer+"/set-function"
            ,{
              id:item.id
              ,func: this.func
            },true)
  ...
  processCommand(btn){
    ...
    if (btn.url){
      if (btn.method==='GET'){
        ...
        this.apiAuth.getDynamicUrl(btn.url,true,httpOptions)
        .then(data=>{
          //console.log(data);
          loading.dismiss();
        ...
        this.apiAuth.postDynamicForm(btn.url, json_data, true)
        .then(data=>{
